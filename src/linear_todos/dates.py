...
class DateParser:
    ...
    @classmethod
    def parse_end_of(cls, input_str: str, base_date: Optional[datetime] = None) -> Optional[str]:
        """Parse 'end of day/week/month' patterns.
        
        Args:
            input_str: String like 'end of day', 'end of week', 'end of month'
        ...
        # Handle "end of day" / "eod" / "by end of day"
        if any(x in input_lower for x in ['end of day', 'eod', 'by end of day', 'by eod']):
            return cls.to_iso_datetime(today.isoformat(), end_of_day=True, tz=tz)
        
        # Handle "end of week" / "eow" / "by end of week"
        if any(x in input_lower for x in ['end of week', 'eow', 'by end of week', 'by eow']):
            # Find Friday (5 = Friday in isoweekday)
            days_until_friday = 5 - today.isoweekday()
            if days_until_friday < 0:
                days_until_friday += 7
            friday = today + timedelta(days=days_until_friday)
            return cls.to_iso_datetime(friday.isoformat(), end_of_day=True, tz=tz)
        
        # Handle "end of month" / "eom" / "by end of month"
        if any(x in input_lower for x in ['end of month', 'eom', 'by end of month', 'by eom']):
            # Get last day of current month
            if today.month == 12:
                last_day = today.replace(year=today.year + 1, month=1, day=1) - timedelta(days=1)
            else:
                last_day = today.replace(month=today.month + 1, day=1) - timedelta(days=1)
            return cls.to_iso_datetime(last_day.isoformat(), end_of_day=True, tz=tz)

...
def run_setup():
    ...
    if click.confirm("Would you like to setup daily cron digests?"):
        from linear_todos.cron_manager import setup_cron_interactive

        # Try to detect timezone from config or USER.md
        tz = None
        if hasattr(config, 'get_timezone'):
            tz_info = config.get_timezone()
            if tz_info:
                tz = str(tz_info)

        setup_cron_interactive(timezone=tz, deliver="origin")
        click.echo()
    ...
    click.echo("  uv run python main.py cron status")
    click.echo()
